...
Determinate::Determinate(double* determinate, int rank)
{
...
void Determinate::rowAddMutiply(int i1, int i2, double times)
{
	cpyDetToDevice();
...
void Determinate::rowNumMutiply(int i1, double times)
{
	cpyDetToDevice();
...
{
	int size = rank * rank * sizeof(double);
...
{
	int id1 = (i1 - 1) * rank + threadIdx.x;
	int id2 = (i2 - 1) * rank + threadIdx.x;
	double temp = det[id1];
	det[id1] = det[id2];
	det[id2] = temp;
}
...
{
	int rowToReduceId = i1 + 1 + threadIdx.x;
