...
int main() {
    ...
    {
        BandGrid<ScalarType> grid(cutEnergy, Si.reciprocal().getLattice(), 0, 0, 0);
        auto solver = KSSolver<ScalarType, LDA<ScalarType, LDAType::HL>>(Si, cutEnergy, std::move(grid), 100, 100, 100);
        solver.solve(1E-3, 100);
        const auto& band = solver.getBandGrid();
        Vector<ComplexType> delta = abs(band[0].getBandUp().getEigenvalues() - data);
