...
int main() {
    ...
    {
        BandGrid<ScalarType, true> grid(Si.reciprocal().getLattice(), 1, 1, 1, 14);
        auto solver = KSSolver<ScalarType, LDA<ScalarType, LDAType::HL, true>>(Si, cutEnergy, 50, std::move(grid));
        solver.solve(1E-3, 100);
        const auto& band = solver.getBand();
        Vector<ComplexType> delta = abs(band.getKPoints()[0].getBandEnergy(SpinState::Up) - data);
