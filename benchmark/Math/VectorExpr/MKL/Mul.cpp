...
namespace {
    ...
    void mul(benchmark::State& state) {
        ...
        for (auto _ : state) {
            hadamard(a, b).assign_mkl(buffer);
...
BENCHMARK(mul<float32>)->Name("mul float32 mkl")->Arg(0)->Arg(1)->Arg(2)->Arg(3)->Arg(4)->Arg(5);
BENCHMARK(mul<float64>)->Name("mul float64 mkl")->Arg(0)->Arg(1)->Arg(2)->Arg(3)->Arg(4)->Arg(5);
