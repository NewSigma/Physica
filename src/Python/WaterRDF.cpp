...
void run(double temperatureT_, double thermostatTime_, double timeStep_) {
    const ScalarType temperatureT = PhyConst<AU>::kToTemperature(temperatureT_);
    const ScalarType thermostatTime = PhyConst<AU>::secondToTime(thermostatTime_);
    const ScalarType timeStep = PhyConst<AU>::secondToTime(timeStep_);
    ...
    {
        const auto rdf = calcRDF<Dynamic>(temperatureT, thermostatTime, timeStep, 32);
    ...
    {
        const auto rdf = calcRDF<1>(temperatureT, thermostatTime, timeStep, 1);
        const VectorType dists = rdf.makeDists() * ScalarType(PhyConst<AU>::bohrToAngstorm(1));
        const VectorType rdfLine = rdf.makeRDF();
        auto& line = plot->line(dists, rdfLine);
        line.setName("MD");
...
PYBIND11_MODULE(PhysicaPython, m) {
    ...
    m.def("run", &run, "Calculate RDF of water");
}
