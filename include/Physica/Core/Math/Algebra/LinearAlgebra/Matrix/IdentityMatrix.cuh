...
namespace Physica {
    ...
    template<Scalar T, size_t Order>
    device_obj<IdentityMatrix<T, Order>>::device_obj(const Matrix auto& m) : device_obj(m.getOrder()) {}
    ...
    void device_obj<IdentityMatrix<T, Order>>::assign(M&& target) const {
        ...
        auto setones = [target_ = asStruct(target)] __device__() mutable {
            ...
            if (i < target.getOrder())
                target[i, i] = T(1);
        };

        constexpr int WarpSize = CUDADevAttr::WarpSize;
        const int numBlock = (getOrder() + (WarpSize - 1)) / WarpSize;
    ...
    template<Scalar T, size_t Order>
    __device__ T device_obj<IdentityMatrix<T, Order>>::calc(size_t row, size_t col, instanceof_x<ThreadBlock> auto) const {
        return calc(row, col);
    }
    ...
    template<Scalar T, size_t Order>
    __host__ __device__ size_t device_obj<IdentityMatrix<T, Order>>::getOrder() const noexcept {
