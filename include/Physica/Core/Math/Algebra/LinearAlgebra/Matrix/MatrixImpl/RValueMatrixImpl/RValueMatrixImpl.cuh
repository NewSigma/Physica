...
namespace Physica {
    ...
    __host__ __device__ auto device_obj<RValueMatrix<Derived, ScalarT>>::diag(this auto&& self) noexcept {
        using Self = decltype(self);
        using M = remove_device_obj<Self>::type;
        return device_obj<MainDiag<M>>(std::forward<Self>(self));
    }

    template<class Derived, Scalar ScalarT>
    __host__ __device__ auto device_obj<RValueMatrix<Derived, ScalarT>>::diag(this auto&& self, ssize_t offset) noexcept {
        using Self = decltype(self);
        using M = remove_device_obj<Self>::type;
        return device_obj<OffsetDiag<M>>(std::forward<Self>(self), offset);
    }
    ...
    template<class Derived, Scalar ScalarT>
    __host__ __device__ consteval bool device_obj<RValueMatrix<Derived, ScalarT>>::isStrided() noexcept {
