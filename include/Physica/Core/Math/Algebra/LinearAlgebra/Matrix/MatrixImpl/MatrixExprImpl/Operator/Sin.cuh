...
namespace Physica {
    ...
    class device_obj<MatrixExpr<ExprID::Sin, M>>
            : public device_obj<UnaryMatrixExpr<ExprID::Sin, M>> {
        using Base = device_obj<UnaryMatrixExpr<ExprID::Sin, M>>;
    public:
    ...
    template<Matrix M>
    __device__ auto device_obj<MatrixExpr<ExprID::Sin, M>>::calc(size_t row, size_t col, instanceof_x<ThreadBlock> auto block) const -> T {
        if constexpr (isReverseDiff())
            return Base::calc_value(row, col, block);
        else
            return sin(Base::getExpr().calc(row, col, block));
    }

    template<Matrix M>
    auto device_obj<MatrixExpr<ExprID::Sin, M>>::values(this auto&& self) noexcept {
        using Self = decltype(self);
        return sin_elem(std::forward<Self>(self).getExpr().values());
    }
