...
namespace Physica {
    ...
    template<Matrix M>
    auto MatrixExpr<ExprID::Relu, M>::values(this auto&& self) noexcept {
    ...
    template<Matrix M>
    [[nodiscard, gnu::always_inline]] auto relu_elem(M&& m) noexcept requires(!DeviceObj<M>) {
        return MatrixExpr<ExprID::Relu, M&&>(std::forward<M>(m));
    }
}
