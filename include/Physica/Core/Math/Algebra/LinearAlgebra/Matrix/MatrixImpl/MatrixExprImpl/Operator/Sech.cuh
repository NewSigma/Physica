...
namespace Physica {
    ...
    class device_obj<MatrixExpr<ExprID::Sech, M>>
            : public device_obj<UnaryMatrixExpr<ExprID::Sech, M>> {
        using Base = device_obj<UnaryMatrixExpr<ExprID::Sech, M>>;
    public:
    ...
    template<Matrix M>
    __device__ auto device_obj<MatrixExpr<ExprID::Sech, M>>::calc(size_t row, size_t col, instanceof_x<ThreadBlock> auto block) const -> T {
        if constexpr (isReverseDiff())
            return Base::calc_value(row, col, block);
        else
            return sech(Base::getExpr().calc(row, col, block));
    }
