...
namespace Physica {
    ...
    class MatrixExpr<ExprType::Sqrt, T>
        ...
        using Base = UnitaryMatrixExpr<ExprType::Sqrt, T>;
    public:
    ...
    template<Matrix T>
    [[nodiscard]] inline auto sqrt_elem(T&& m) noexcept requires(!CUDA<T>) {
        return MatrixExpr<ExprType::Sqrt, T&&>(std::forward<T>(m));
    }
}
