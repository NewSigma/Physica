    ...
    void DenseMatrix<T, Option, Row, Column, MaxRow, MaxColumn, Allocator>::read(
        ...
        for (size_t i = 0; i < Base::getMaxMajor(); ++i) {
            if constexpr (MatrixOption::isRowMatrix<This>()) {
                auto row = Base::row(i);
                file_space.selectHyperslab(H5S_seloper_t::H5S_SELECT_SET, {1, getColumn()}, {i, 0});
                dataset.read(row.data(), ScalarType::getH5DataType(), mem_space, file_space.asH5Type(), xfer_plist);
            }
            else {
                auto col = Base::col(i);
                file_space.selectHyperslab(H5S_seloper_t::H5S_SELECT_SET, {getRow(), 1}, {0, i});
                dataset.read(col.data(), ScalarType::getH5DataType(), mem_space, file_space.asH5Type(), xfer_plist);
            }
        }
    }
    ...
    void DenseMatrix<T, Option, Row, Column, MaxRow, MaxColumn, Allocator>::write(
        ...
        for (size_t i = 0; i < Base::getMaxMajor(); ++i) {
            if constexpr (MatrixOption::isRowMatrix<This>()) {
                ...
                file_space.selectHyperslab(H5S_seloper_t::H5S_SELECT_SET, {1, getColumn()}, {i, 0});
                dataset.write(row.data(), ScalarType::getH5DataType(), mem_space, file_space.asH5Type(), xfer_plist);
            }
            else {
                const auto col = Base::col(i);
                file_space.selectHyperslab(H5S_seloper_t::H5S_SELECT_SET, {getRow(), 1}, {0, i});
                dataset.write(col.data(), ScalarType::getH5DataType(), mem_space, file_space.asH5Type(), xfer_plist);
            }
        }
    }
