...
namespace Physica {
    ...
    public:
        using Base::Base;
        /* Operations */
        [[nodiscard]] __device__ T calc(size_t index) const { return sin(Base::getExpr().calc(index)); }

        [[nodiscard]] __device__ Tv calc_value(size_t index) const { return sin(Base::getExpr().calc_value(index)); }
    };
