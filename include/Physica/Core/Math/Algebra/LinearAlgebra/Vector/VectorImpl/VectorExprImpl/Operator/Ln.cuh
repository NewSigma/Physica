...
namespace Physica {
    template<Vector V>
    class device_obj<VectorExpr<ExprID::Ln, V>> : public device_obj<UnitaryVectorExpr<ExprID::Ln, V>> {
        using This = device_obj<VectorExpr<ExprID::Ln, V>>;
        using Base = device_obj<UnitaryVectorExpr<ExprID::Ln, V>>;
    public:
    ...
    template<Vector V>
    __device__ auto device_obj<VectorExpr<ExprID::Ln, V>>::calc(size_t index, instanceof_x<ThreadBlock> auto block) const -> T {
        if constexpr (isReverseDiff())
            return Base::calc_value(index, block);
        else
            return ln(Base::getExpr().calc(index, block));
    }

    template<Vector V>
    auto device_obj<VectorExpr<ExprID::Ln, V>>::values(this auto&& self) noexcept {
