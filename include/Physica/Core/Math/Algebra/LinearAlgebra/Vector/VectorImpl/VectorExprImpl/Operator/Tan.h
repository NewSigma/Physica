...
namespace Physica {
    ...
    template<Vector V>
    template<Packet Pack>
    Pack VectorExpr<ExprID::Tan, V>::operator()(std::random_access_iterator auto input) noexcept {
        return tan(input.template load<Pack>());
    }

    template<Vector V>
    template<Packet Pack>
    Pack VectorExpr<ExprID::Tan, V>::operator()(std::random_access_iterator auto input, size_t count) noexcept {
        return tan(input.template load<Pack>(count));
    }
