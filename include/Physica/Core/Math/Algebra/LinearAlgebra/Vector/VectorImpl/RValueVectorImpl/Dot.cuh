...
namespace Physica {
    ...
    private:
        ...
        const V2& v2;
    public:
        __device__ device_obj(const V1& v1_, const V2& v2_);
        ...
        /* Operations */
        __device__ auto calc() const noexcept;
        __device__ auto calc(instanceof_x<ThreadBlock> auto block) const noexcept;
    };

    template<Vector V1, Vector V2>
    __device__ device_obj<Dot<V1, V2>>::device_obj(const V1& v1_, const V2& v2_) : v1(v1_), v2(v2_) {
        assert(v1.getLength() == v2.getLength());
    }

    template<Vector V1, Vector V2>
    __device__ auto device_obj<Dot<V1, V2>>::calc() const noexcept {
        return calc(ThreadBlock<1>{});
    }

    template<Vector V1, Vector V2>
    __device__ auto device_obj<Dot<V1, V2>>::calc(instanceof_x<ThreadBlock> auto block) const noexcept {
        T dot = 0;
    ...
    template<Vector V1, Vector V2>
    __device__ auto dot(const V1& v1, const V2& v2) requires(DeviceObj<V1> && DeviceObj<V2>) {
