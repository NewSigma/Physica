...
namespace Physica {
    ...
    public:
        ...
        [[nodiscard]] Pack packet(size_t index) const noexcept { return sqrt(Base::getExpr().template packet<Pack>(index)); }

        template<Packet Pack>
        [[nodiscard]] Pack packet(size_t index, size_t count) const noexcept { return sqrt(Base::getExpr().template packet<Pack>(index, count)); }
    };
