...
namespace Physica {
    ...
    class EnergyMinimizer {
        using This = EnergyMinimizer<T, Dim>;
    public:
    ...
    public:
        ...
        EnergyMinimizer(const This&) = default;
        EnergyMinimizer(This&&) noexcept = default;
        ~EnergyMinimizer() = default;
        /* Operators */
        This& operator=(This obj) noexcept { swap(obj); return *this; }
        /* Operations */
        template<ExecutePolicy P>
        void pre_pos_step(const auto& model, auto& optimizer);
        template<ExecutePolicy P>
        void pos_step(const auto& model, auto& optimizer);
        template<ExecutePolicy P>
        void pre_lattice2D_step(const auto& model, auto& optimizer, T diffStep);
        template<ExecutePolicy P>
        void lattice2D_step(const auto& model, auto& optimizer, T diffStep);
        void swap(This& __restrict obj) noexcept;
        /* Getters */
        [[nodiscard]] const MDCellType& getCell() const noexcept { return cell; }
    private:
        auto makePosStepFunc(const auto& model);
        template<ExecutePolicy P>
        auto makePosStepGrad(const auto& model);
        auto makeLattice2DStepFunc(const auto& model);
        auto makeLattice2DStepGrad(const auto& model, T diffStep);
    };
    ...
    template<Scalar T, unsigned int Dim>
    template<ExecutePolicy P>
    void EnergyMinimizer<T, Dim>::pre_pos_step(const auto& forceModel, auto& optimizer) {
        const auto func = makePosStepFunc(forceModel);
        const auto grad = makePosStepGrad<P>(forceModel);
        optimizer.init(cell.getPos().flatten(), func, grad);
    }

    template<Scalar T, unsigned int Dim>
    template<ExecutePolicy P>
    void EnergyMinimizer<T, Dim>::pos_step(const auto& model, auto& optimizer) {
    ...
    template<Scalar T, unsigned int Dim>
    template<ExecutePolicy P>
    void EnergyMinimizer<T, Dim>::lattice2D_step(const auto& forceModel, auto& optimizer, T diffStep) {
        assert(optimizer.getDim() == 3 && "[Error]: pre_lattice2D_step must be called before this function");
        const auto func = makeLattice2DStepFunc(forceModel);
        const auto grad = makeLattice2DStepGrad(forceModel, diffStep);
        optimizer.step(func, grad);
    ...
    template<Scalar T, unsigned int Dim>
    void EnergyMinimizer<T, Dim>::swap(This& __restrict obj) noexcept {
    ...
    template<Scalar T, unsigned int Dim>
    auto EnergyMinimizer<T, Dim>::makeLattice2DStepGrad(const auto& forceModel, T diffStep) {
        const auto func = makeLattice2DStepFunc(forceModel);
