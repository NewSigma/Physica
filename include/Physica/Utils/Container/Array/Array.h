...
namespace Physica {
    template<class T, size_t Length, class Allocator>
    class Traits<Utils::Array<T, Length, Allocator>> {
    public:
        using ValueType = T;
...
namespace std {
    template<class T, size_t Length, class Allocator>
    inline void swap(Physica::Utils::Array<T, Length, Allocator>& __restrict array1,
                     Physica::Utils::Array<T, Length, Allocator>& __restrict array2) noexcept {
        array1.swap(array2);
    }
}
