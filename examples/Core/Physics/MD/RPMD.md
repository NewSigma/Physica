...
$$Q = \frac{1}{(2\pi\hbar)^N} \int e^{-\beta H(\mathbf{p, q})} \text{d}\mathbf{p}\text{d}\mathbf{q}$$
 
PIMD相当于在量子玻尔兹曼分布中进行采样，将积分改为求迹，将哈密顿量改为哈密顿算符可得
...
$$Q \approx \frac{1}{(2\pi\hbar)^{Nn}} \int \text{d}\mathbf{q} \text{d}\mathbf{p} e^{-\beta_n H_n}$$
...
即如不考虑耦合项，$H_n$将分成$n$个完全独立的经典系统。量子效应则将这$n$个系统用弹簧耦合。
...
一般来说$\hbar$是一个相对很小的量，这导致$\hbar$作为分母的弹性常数 $k$很大，弹簧近乎刚性。因此被弹簧连接的粒子将局域在一个很小的空间范围内。由于 $k \propto T^2$，温度越高弹性系数越大，核量子效应越不显著。在高温情况下，可以近似认为弹簧几乎不会伸长，弹簧环退化为质点，量子将过渡到经典情况。

在理解了PIMD的物理图像后，我们重新回顾上述理论框架。在量子力学的路径积分表述中，从初态到末态的所有可能路径都使用一个作用量的指数因子$\exp(\frac{i}{\hbar} S)$进行加权求和。但是在上面的推导中，我们完全没有用到系统的作用量 $S$。因此PIMD中的路径积分并非“物理”的路径积分，而单纯只是一种计算配分函数的数学技巧。它只能给出近似的量子动力学。在前文讲述的平行宇宙图像中，我们对所有粒子进行了序号标记。这意味着我们先验地假设了粒子是可分辨的，即忽略了粒子间的交换相互作用。这个近似在温度不十分低的凝聚态体系中可以适用，但是在温度达到十几K甚至更低的时候就必须考虑粒子间的交换相互作用。近几年关于费米子和玻色子的PIMD理论也得到一些发展，感兴趣的读者可以关注。
