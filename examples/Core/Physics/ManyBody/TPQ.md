...
热力学纯态(Thermo Pure Quantum, TPQ)$^{[1]}$, 用于计算有限温多体系统的各种力学量和热力学量。随着系统格点数增加，计算结果依概率以指数速度收敛到热力学极限。这种收敛是相当快的，实际上4格点的条件下已与精确对角化结果非常接近。TPQ相比精确对角化的改进在于，将计算有限温所需的$O(N)$个本征向量减少到1个随机向量，极大地减少了内存需求。
